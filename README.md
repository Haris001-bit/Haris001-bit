...
**Multi-tenant RAG platform.** Architected retrieval for a compliance/GRC product: hybrid dense + lexical retrieval, cross-encoder reranking, role-scoped knowledge isolation, and a tiered latency pipeline that cut 3–8 seconds from every follow-up message.
...
These are engineering write-ups rather than client deliverables. Client work is published only with written permission, and never includes source, credentials, or customer data — a standard I'd apply to your project too.
